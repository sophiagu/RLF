...
def train(agent, is_evaluation, model_params=None):
  ...
  if agent == 'ppo2':
    model = PPO2(MlpLstmPolicy, envs, nminibatches=1, verbose=1) if model_params is None\
    else PPO2(MlpLstmPolicy, envs, nminibatches=1, verbose=1, **model_params)
  elif agent == 'a2c':
    model = A2C(MlpLstmPolicy, envs, verbose=1) if model_params is None\
    else A2C(MlpLstmPolicy, envs, verbose=1, **model_params)
  else:
...
def search_hparams(agent, trial):
  ...
  model = train(agent, True, model_params)
